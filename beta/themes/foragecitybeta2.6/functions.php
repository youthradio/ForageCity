...
function do_this_hourly() {
	...
	$boxes = get_posts(array("post_type" => "fcboxes", 'numberposts' => -1, 'post_status' => null, 'orderby' => 'post_date', 'order' => 'ASC'));
	reset($boxes);
	$rp = current($boxes);
...
<result>';
	...
	$good_description = fc_get_form_value('good_description');
...
'.$pics[$gtp].'
		...
		<message>Valid location required.</message>";
			...
			$new_box_data = create_box_post_ajax($box_number, $instructions, $good_description, $quantity, $location, $good_type_post, $good_units, $geocode);
...
$x
	...
	<box>
		...
		<good_description>".htmlentities($good_description)."</good_description>
		<pic>
			<![CDATA[
".$pic."
			]]>
...
function create_box_post_ajax($box_number, $instructions, $good_description, $quantity, $location, $good_type_post, $good_units, $geocode){
	$result = false;
	$new_box = wp_insert_post(array(
		// 'ID' => '12',
		'post_title' => $box_number,
		'post_content' => $good_description,
