var myScroll,fixScroll,tm=false,top_btn=false,back1,back2,back=0,keyTO,debug,debug_input,debug_xhr,debug_result,hu=HOME_URL,map,sll=false,zm=false,slat,slong,scrollOff=false,hasTouch='ontouchstart' in window,tbtm=false,iOS=true,taplock=false,tlTO,lastgn,ff=false,perListPage=20,geoLoc=false,defaultSort='wishlist',extras,//lTO,
	...
	addToMap = function(map,lat,lng,boxpost,good_description,type,input){
		var marker = new google.maps.Marker({
			position: new google.maps.LatLng(lat,lng),
			title:good_description
		});
			...
			success: function(data, status, xhr){
				...
				if(input.action == "fc_load_find") {
					...
					// DISPLAY DETAILS OF THE GIVEN BOX
						...
						g_desc = box.find('good_description').text();
						...
						if(g_desc != "")
							newBoxHTML += '<h3>Description</h3><p>'+g_desc+'</p>';
				...
				// BASKET FUNCTIONS
					...
					// DISPLAY DETAILED INFO FOR GIVEN RESERVATION
						...
						if(instr != "")
							newBoxHTML += '<h3>Instructionss</h3><p>'+instr+'</p>';
				...
				// GIVE FUNCTIONS
					...
					// CREATE A NEW BOX
						...
						if(desc != "")
							newHTML += '<div id="box-good_description"><p class="strong">Description</p><p>'+desc+'</p></div>';
						newHTML += '<hr><div id="what-to-do"><p>Write this number on your container of goods, then press share so that they can be publicly listed.  Please also mark the box with foragecity.com.</p><p id="box-number">'+bn+'</p><form action="'+hu+'/give/" method="post">';
				...
				// PROFILE FUNCTIONS
					...
					// VIEW/MODIFY A SHARED BOX
						...
						instr = box.find("instructions").text();
						desc = box.find("good_description").text();
		...
		sw.delegate("input[type=submit]", clte, function(e, undefined){
			...
			if(claimTapLock(this) && !tm){
				...
				if(t.is("div.find-box div.box-info input")){
					...
					instv = inst.val();
					desc = form.find("input[name=good_description]");
					descv = desc.val();
				   ...
				   get_wp_data('GET', data);
					...
					data = {
						...
						good_description: form.find("textarea[name=good_description]").val(),
