...
function FCFacebookBoxAnnounce($box_post, $good_info) {
	$gi = "something";
	if(!empty($good_info))
		$gi = $good_info;
...
add_action('fc_box_shared','FCFacebookBoxAnnounce', 10, 2);
...
function FCFacebookProfileExtras(){ ?>
	...
	if ($user) {
		try {
			...
			update_user_meta( $user_id, 'facebook_token', $facebook->getAccessToken() );
			$success = true;
			echo "<span style='width:100%;text-align:center;'><a target='_blank' href='{$user_profile["link"]}'><img style='vertical-align:-5px;width:1.25em;height:1.25em;margin:0 3px;' src='https://graph.facebook.com/$user/picture'>{$user_profile["name"]}</a></span> Your Facebook account is linked to your Forage City account.";
/*	     <a href="<?php echo $facebook->getLogoutUrl(); ?>">Logout of Facebook</a> */
		} catch (FacebookApiException $e) {
			error_log($e);
			$user = null;
	...
	if(!$success) { ?>
		<a href="<?php echo $facebook->getLoginUrl(array('scope' => 'offline_access')); ?>">Login with Facebook</a><?php
	} ?>
	</span></td></tr><?php
}
