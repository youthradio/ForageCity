...
<?php
	...
	if($success || !(empty($user_token) || empty($user_secret))) {
		try{
			...
			echo "<span style='width:100%;text-align:center;'><a target='_blank' href='https://twitter.com/#!/{$twitterInfo->screen_name}'><img style='vertical-align:-5px;width:1.25em;height:1.25em;margin:0 3px;' src='{$twitterInfo->profile_image_url}'> {$twitterInfo->screen_name}</a></span> <span style='width:100%;text-align:center;'>Your Twitter account is linked.</span>";
			$success = true;
		} catch(Exception $e){ $success = false; }
	}
	if(!$success){
		echo '<span style="width:100%;text-align:center;"><a target="_blank" href="' . $authURL . '">Authorize with Twitter</a></span>';
	} ?>
	</span></td></tr><?php
}
...
function FCTwitterGiveExtras($extras){
	...
	$user_secret = get_user_meta( $user_id, 'twitter_token_secret', true );
	if(!empty($user_token) && !empty($user_secret))
		$extras['post_to_twitter'] = 'Let your Twitter followers know about these goods';
	return $extras;
}
